...
namespace VostokVR.Geo {
    ...
    public class SphereScript : MonoBehaviour {
        ...
        [Range(0.1f, 100.0f)]
        //[HideInInspector, Tooltip("Sphere Radius")]
        public float Radius = 20f;

        [Range(2, 6)]
        //[HideInInspector, Tooltip("Detail level. The higher, the more triangles.")]
        public int Detail = 2;

        [HideInInspector, Tooltip("Whether to use an IcoSphere or UV sphere. IcoSphere are better for planets. Both use UV coordinates properly.")]
        ...
        public void ChangeSize( float new_radius ) {

            dirty = false;

            Radius = lastRadius = new_radius;

            #if UNITY_EDITOR
            if( IcoSphere ) CreateIcoSphere();
            else CreateUVSphere();
            #endif

            if( GetComponent<SphereCollider>() != null ) GetComponent<SphereCollider>().radius = new_radius;
        }
    }
}